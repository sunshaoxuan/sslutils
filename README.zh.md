...
- **基于旧证书续期**: `New-CertificateSigningRequestFromOld.ps1` (自动从 old 目录读取信息)。
  全新交互模式会先询问 `CN`，再补齐 `C / ST / L / O`，最后才进入 SAN 设置。默认先生成到 `new/<CN>/`，生成完成后立即只针对这个域名调用一次文件夹重命名逻辑，因此通常可以直接落到最终的 `机构名 (主机名)` 目录，而不用等下次启动工具。

### 4. 私钥管理
- **解密/去密**: `Convert-KeyToPlaintext.ps1` (将加密的私钥转换为无需密码的 RSA Key)。
  也就是把“带密码的 `.key`”转换成“可直接部署的无密码 key”。如果已经存在 `<原名>.decrypted.key`，脚本会在执行过程中询问是否先备份再覆盖。
...
现在新建 CSR 时也会在生成后立即复用同一套重命名逻辑，但只针对本次新建的域名目录执行一次，这样新生成的文件夹通常会当场改成最终的机构名称格式。
